...
[使用文档](https://kubernetes.io/docs/home/)
## 离线包地址
使用前注意个版本的依赖
...
百度网盘地址：
```
链接：https://pan.baidu.com/s/1xUazz8mNjcwRWVvGOox_Kw 密码：p011
用find找出包里面的tar后缀的docker镜像, docker load < xx.tar
...
关闭服务
```
screen -ls|grep [.]kube|awk -F'.' '{print $1}'|column -t|xargs  -I@ kill -5 @&&screen -wipe
```
...
kubectl describe pod existing-dachshund-mysql-f959967c7-pkqb5
```
...
http_proxy=http://192.168.1.213:8118 helm init --service-account tiller  --tiller-image=gcr.io/kubernetes-helm/tiller:canary --stable-repo-url=http://kubernetes-charts.storage.googleapis.com
}
```
...
ingress 使用helm安装
```
wget https://raw.githubusercontent.com/kubernetes/ingress-nginx/master/deploy/mandatory.yaml -O ingress-nginx-mandatory.yaml
kubectl create -f ingress-nginx-mandatory.yaml
附：
https://kubernetes.github.io/ingress-nginx/deploy/
http_proxy=http://192.168.1.213:8118 helm install stable/nginx-ingress --name my-release --set controller.stats.enabled=true
```

开启权限控制RBAC【进行中】
```
mkdir /opt/kubernetes/cert/&& cd /opt/kubernetes/cert/
创建证书
ca证书:
openssl genrsa -out ca.key 2048
openssl req -x509 -nodes -key ca.key -subj "/CN=zh" -days 5000 -out ca.crt
服务端证书
...
/opt/kubernetes/server/kube-apiserver.sh:
bin/kube-apiserver  --insecure-bind-address=0.0.0.0 --secure-port=6443 --service-cluster-ip-range=10.0.0.0/16 --etcd-servers=http://192.168.1.234:2379 --logtostderr=true  --allow-privileged=true --anonymous-auth=false --authorization-mode=RBAC --kubelet-client-certificate=/opt/kubernetes/cert/client.crt --kubelet-client-key=/opt/kubernetes/cert/client.key --client-ca-file=/opt/kubernetes/cert/ca.crt --tls-private-key-file=/opt/kubernetes/cert/server.key --tls-cert-file=/opt/kubernetes/cert/server.crt --service-account-key-file=/opt/kubernetes/cert/server.key
/opt/kubernetes/node/kubelet.sh:
bin/kubelet    --kubeconfig=etc/kubernetes/kubelet.kubeconfig  --config=etc/kubernetes/kubelet.config.yaml --pod-infra-container-image=registry.access.redhat.com/rhel7/pod-infrastructure:latest  --runtime-cgroups=/systemd/system.slice --kubelet-cgroups=/systemd/system.slice  --logtostderr=true  --fail-swap-on=false --allow_privileged=true --cluster_dns=10.0.244.1 --cluster_domain=cluster.local --anonymous-auth=false --tls-private-key-file=/opt/kubernetes/cert/client.key --tls-cert-file=/opt/kubernetes/cert/client.crt
...
{
cd /opt/kubernetes/node
KUBE_APISERVER="https://192.168.1.234:6443"
kubectl config set-cluster kubernetes --server=$KUBE_APISERVER --kubeconfig=etc/kubernetes/kubelet.kubeconfig --client-key=/opt/kubernetes/cert/client.key --certificate-authority=/opt/kubernetes/cert/ca.crt 
kubectl config set-credentials default-noauth --certificate-authority=/opt/kubernetes/cert/ca.crt  --client-key=/opt/kubernetes/cert/client.key --client-certificate=/opt/kubernetes/cert/client.crt
kubectl config set-context default --cluster=kubernetes --user=default-noauth --kubeconfig=etc/kubernetes/kubelet.kubeconfig
kubectl config use-context default --kubeconfig=etc/kubernetes/kubelet.kubeconfig
kubectl config view --kubeconfig=etc/kubernetes/kubelet.kubeconfig
}


测试:
yum install -y mod_ssl
curl https://master:6443/ --cacert /opt/kubernetes/cert/ca.crt --key /opt/kubernetes/cert/client.key --cert /opt/kubernetes/cert/client.crt 
in   default-noauth-rbac.yaml
...
kubectl create -f default-noauth-rbac.yaml
